...
- Calculate the average salary earned by professors in the Political Science department.
...
In R, create three vectors each of length 10: `v1` equals the number 1 repeated 10 times; `v2` equals the numbers 1 to 10; and `v3` equals the sequence $1^2=2$, $2^2=4$, $3^2=9$, $\ldots$, $10^2=100$. Think of each of these vectors as respondents in a survey (or an observation of, say, a country) *not* as variables in a dataset (`v1`,`v2`,`v3` are not columns/variables). So in the context of a dataset/dataframe our data would look like:
...
| v3 |  1  |  4  |  9  |  16 |  25 |  36 |  49 |  64 |  81 |  100|

You can think of the weights as being applied to each observation (like survey weights). So for example, for part 3) below, the weight vector would look like `w1 = c(1,2,3)` where `v1` (observation 1/respondent 1/country 1) has a weight of 1, `v2` (observation 2/respondent 2/country 2) has a weight of 2, `v3` (observation 3/respondent 3/country 3) has a weight of 3. This means that all of the values for each `x1` to `x10` for each vector is weighted by these values.

Compute the following weighted averages of these three vectors. In each case, your output should be a vector of length 10 where each value corresponds to weighted averages for all of the variables/covariates/columns of the dataframe above.
...
1.  **Data visualization** (Exercise 2.6 of *Regression and Other Stories*). Take data from some problem of interest to you and make several plots to highlight different aspects of the data, as was done in the baby-names example in Figures 2.6–2.8 (or how we summarized data in class, including the handedness survey).

::: callout-note
#### Hint:

For some useful code for plotting densities, check out my post on Ed that includes the code for making (overlapping) density plots.

Also remember that you do not need to replicate these figures, you just need to display the data to highlight interesting aspects of the data (e.g., skew, differences between groups).
